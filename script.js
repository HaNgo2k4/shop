...
localStorage.setItem('online-store', JSON.stringify(user.cart));
const selectors = {
...
document.querySelector(".checkout").addEventListener("click", async () =>{
  ...
  // localStorage.setItem('user'.ca, JSON.stringify(JSON.parse(localStorage.getItem('online-store'))));
  // user.cart = JSON.parse(localStorage.getItem('online-store'))
 ...
 const send_data = async() =>{
  ...
  fetch("https://d7gcxr-3000.csb.app/user/"+ user.id, options)
...
//* event handlers
//https://d7gcxr-3000.csb.app/item?_sort=id&_order=desc&_page=1&_limit=2
const initStore = () => {
  loadCart();
  loadProducts("https://d7gcxr-3000.csb.app/item?_sort=id&_order=desc&iduser_ne="+ user.id)
    .then(renderProducts)
    .finally(renderCart);
};
...
setupListeners();
// alert(localStorage.getItem('online-store'))